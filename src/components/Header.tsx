...
export default function Header() {
  ...
  return (
    <>
      {/* Category Nav — above header */}
      <div className="border-b bg-white" style={{ borderColor: 'var(--c-border)' }}>
        <div className="site-container py-2">
          <div className="flex items-center gap-1 sm:gap-4 overflow-x-auto scrollbar-hide text-sm">
            <Link href="/"
              className="whitespace-nowrap px-3 py-1 rounded-md font-bold transition-colors"
              style={{ color: 'var(--c-text)', backgroundColor: 'var(--c-surface)' }}>推荐</Link>
            {NAV.map(cat => (
              <Link key={cat.slug} href={`/category/${cat.slug}`}
                className="whitespace-nowrap px-3 py-1 rounded-md transition-colors hover:bg-gray-50"
      ...
      <header className="sticky top-0 z-50 bg-white border-b" style={{ borderColor: 'var(--c-border)' }}>
        <div className="site-container flex items-center justify-between h-14 sm:h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden w-8 h-8 flex items-center justify-center" aria-label="菜单" style={{ color: 'var(--c-text)' }}>
          ...
          <div className="md:hidden border-t px-4 py-3 space-y-1" style={{ backgroundColor: 'var(--c-bg)', borderColor: 'var(--c-border)' }}>
            {NAV.map(cat => (
              <Link key={cat.slug} href={`/category/${cat.slug}`} onClick={() => setMenuOpen(false)}
                className="block py-2 text-sm border-b last:border-0" style={{ borderColor: 'var(--c-border)', color: 'var(--c-text)' }}>{cat.name}</Link>
            ))}
          </div>
        )}
      </header>
